...
namespace Petboard
{
    ...
    public sealed class SimpleDbMembershipProvider : MembershipProvider
    {
        ...
        static readonly List<string> PasswordStrings = new List<string>(){ "Password", "PasswordAnswer" };
        ...
        public override MembershipUser CreateUser(string userName, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
        {
            ...
            if (user == null)
            {
                ...
                if (String.IsNullOrEmpty(existingUser))
                {
                    ...
                    data.Add(new ReplaceableAttribute() { Name = "Email", Value = email });
                    data.Add(new ReplaceableAttribute() { Name = "Password", Value = password });
                    if (passwordQuestion != null)
                    {
                        data.Add(new ReplaceableAttribute() { Name = "PasswordQuestion", Value = passwordQuestion });
                    }

                    if (passwordAnswer != null)
                    {
                        data.Add(new ReplaceableAttribute() { Name = "PasswordAnswer", Value = passwordAnswer });
                    }

                    data.Add(new ReplaceableAttribute() { Name = "IsApproved", Value = isApproved.ToString() });
                    PutAttributesRequest request = new PutAttributesRequest()
                    {
                        DomainName = Settings.Default.AWSMembershipDomain,
                        ItemName = userName
        ...
        public override MembershipUserCollection GetAllUsers(int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            SelectRequest request = new SelectRequest(){SelectExpression = (String.Concat("Select * from `", Settings.Default.AWSMembershipDomain, "`"))};

            SelectResponse response = this._simpleDBClient.Select(request);

            totalRecords = response.Items.Count;
        ...
        public override string ResetPassword(string userName, string answer)
        {
            ...
            PutAttributesRequest prequest = new PutAttributesRequest()
            {
                DomainName = Settings.Default.AWSMembershipDomain,
                ItemName = userName,
                Attributes = new List<ReplaceableAttribute>(){replace}
            };
